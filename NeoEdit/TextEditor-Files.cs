...
namespace NeoEdit.Program
{
	partial class TextEditor
	{
		async Task<bool> BinarySearchFileAsync(string fileName, BinarySearcher searcher, IProgress<ProgressReport> progress, CancellationToken cancel)
		{
			...
			catch (Exception ex)
			{
				if (!state.SavedAnswers[nameof(BinarySearchFileAsync)].HasFlag(MessageOptions.All))
					state.TabsWindow.Dispatcher.Invoke(() =>
					{
						state.SavedAnswers[nameof(BinarySearchFileAsync)] = new Message(state.TabsWindow)
						{
							Title = "Confirm",
			...
			catch (Exception ex)
			{
				state.TabsWindow.Dispatcher.Invoke(() =>
				{
					if (!state.SavedAnswers[nameof(TextSearchFileAsync)].HasFlag(MessageOptions.All))
						state.SavedAnswers[nameof(TextSearchFileAsync)] = new Message(state.TabsWindow)
						{
							Title = "Confirm",
							Text = $"Unable to read {fileName}.\n\n{ex.Message}\n\nLeave selected?",
							Options = MessageOptions.YesNoAllCancel,
		...
		void ConfigureExecute_Files_Name_MakeAbsolute() => state.ConfigureExecuteData = FilesNamesMakeAbsoluteRelativeDialog.Run(state.TabsWindow, GetVariables(), true, true);

		void Execute_Files_Name_MakeAbsolute()
		{
			var result = state.ConfigureExecuteData as FilesNamesMakeAbsoluteRelativeDialog.Result;
		...
		void ConfigureExecute_Files_Name_MakeRelative() => state.ConfigureExecuteData = FilesNamesMakeAbsoluteRelativeDialog.Run(state.TabsWindow, GetVariables(), false, true);
		...
		void ConfigureExecute_Files_Name_GetUnique() => state.ConfigureExecuteData = FilesNamesGetUniqueDialog.Run(state.TabsWindow);

		void Execute_Files_Name_GetUnique()
		{
			var result = state.ConfigureExecuteData as FilesNamesGetUniqueDialog.Result;
		...
		void ConfigureExecute_Files_Set_Size()
		{
			...
			state.ConfigureExecuteData = FilesSetSizeDialog.Run(state.TabsWindow, vars);
		}

		void Execute_Files_Set_Size()
		{
			var result = state.ConfigureExecuteData as FilesSetSizeDialog.Result;
			var vars = GetVariables();
			var files = RelativeSelectedFiles();
		...
		void ConfigureExecute_Files_Set_Time() => state.ConfigureExecuteData = FilesSetTimeDialog.Run(state.TabsWindow, GetVariables(), $@"""{DateTime.Now}""");

		void Execute_Files_Set_Time(TimestampType type)
		{
			var result = state.ConfigureExecuteData as FilesSetTimeDialog.Result;
		...
		void ConfigureExecute_Files_Set_Attributes()
		{
			...
			state.ConfigureExecuteData = FilesSetAttributesDialog.Run(state.TabsWindow, current);
		}

		void Execute_Files_Set_Attributes()
		{
			var result = state.ConfigureExecuteData as FilesSetAttributesDialog.Result;
		...
		void ConfigureExecute_Files_Find() => state.ConfigureExecuteData = FilesFindDialog.Run(state.TabsWindow, GetVariables());

		void Execute_Files_Find()
		{
			var result = state.ConfigureExecuteData as FilesFindDialog.Result;
			...
			if (result.IsBinary)
			{
				...
				results = MultiProgressDialog.RunAsync(state.TabsWindow, "Searching files...", RelativeSelectedFiles().Select((file, index) => (file, index)), async (obj, progress, cancel) => await BinarySearchFileAsync(obj.file, searchers[stringsToFind[obj.index]], progress, cancel), obj => Path.GetFileName(obj.file));
			}
			else
			{
				// Create searchers
				...
				results = MultiProgressDialog.RunAsync(state.TabsWindow, "Searching files...", RelativeSelectedFiles().Select((file, index) => (file, index)), async (obj, progress, cancel) => await TextSearchFileAsync(obj.file, searchers[stringsToFind[obj.index]], progress, cancel), obj => Path.GetFileName(obj.file));
			}

			Selections = Selections.Where((range, index) => results[index]).ToList();
		}

		void ConfigureExecute_Files_Insert() => state.ConfigureExecuteData = FilesInsertDialog.Run(state.TabsWindow);

		void Execute_Files_Insert()
		{
			var result = state.ConfigureExecuteData as FilesInsertDialog.Result;
		...
		void ConfigureExecute_Files_Create_FromExpressions() => state.ConfigureExecuteData = FilesCreateFromExpressionsDialog.Run(state.TabsWindow, GetVariables(), CodePage);

		void Execute_Files_Create_FromExpressions()
		{
			var result = state.ConfigureExecuteData as FilesCreateFromExpressionsDialog.Result;
		...
		void ConfigureExecute_Files_Select_ByVersionControlStatus() => state.ConfigureExecuteData = FilesSelectByVersionControlStatusDialog.Run(state.TabsWindow);

		void Execute_Files_Select_ByVersionControlStatus()
		{
			var result = state.ConfigureExecuteData as FilesSelectByVersionControlStatusDialog.Result;
			var versioner = new Versioner();
		...
		void ConfigureExecute_Files_Hash() => state.ConfigureExecuteData = FilesHashDialog.Run(state.TabsWindow);

		void Execute_Files_Hash()
		{
			var result = state.ConfigureExecuteData as FilesHashDialog.Result;
			ReplaceSelections(MultiProgressDialog.RunAsync(state.TabsWindow, "Calculating hashes...", RelativeSelectedFiles(), (file, progress, cancel) => Hasher.GetAsync(file, result.HashType, result.HMACKey, progress, cancel)));
		}

		void ConfigureExecute_Files_Sign() => state.ConfigureExecuteData = FilesSignDialog.Run(state.TabsWindow);

		void Execute_Files_Sign()
		{
			var result = state.ConfigureExecuteData as FilesSignDialog.Result;
			ReplaceSelections(RelativeSelectedFiles().Select(file => Cryptor.Sign(file, result.CryptorType, result.Key, result.Hash)).ToList());
		}

		void ConfigureExecute_Files_Operations_CopyMove(bool move) => state.ConfigureExecuteData = FilesOperationsCopyMoveDialog.Run(state.TabsWindow, GetVariables(), move);

		void Execute_Files_Operations_CopyMove(bool move)
		{
			var result = state.ConfigureExecuteData as FilesOperationsCopyMoveDialog.Result;
			var variables = GetVariables();
			...
			if (invalid.Any())
			{
				if (!state.SavedAnswers[overwriteCopyMove].HasFlag(MessageOptions.All))
					state.SavedAnswers[overwriteCopyMove] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
			...
			foreach (var file in files)
			{
				...
				catch (Exception ex)
				{
					if (!state.SavedAnswers[continueAnswer].HasFlag(MessageOptions.All))
						state.SavedAnswers[continueAnswer] = new Message(state.TabsWindow)
						{
							Title = "Confirm",
		...
		void ConfigureExecute_Files_Operations_Encoding() => state.ConfigureExecuteData = FilesOperationsEncodingDialog.Run(state.TabsWindow);

		void Execute_Files_Operations_Encoding()
		{
			var result = state.ConfigureExecuteData as FilesOperationsEncodingDialog.Result;
			MultiProgressDialog.Run(state.TabsWindow, "Changing encoding...", RelativeSelectedFiles(), (inputFile, progress, cancel) => ReencodeFile(inputFile, progress, cancel, result.InputCodePage, result.OutputCodePage));
		}
		...
		void ConfigureExecute_Files_Operations_SplitFile()
		{
			...
			state.ConfigureExecuteData = FilesOperationsSplitFileDialog.Run(state.TabsWindow, variables);
		}

		void Execute_Files_Operations_SplitFile()
		{
			var result = state.ConfigureExecuteData as FilesOperationsSplitFileDialog.Result;
			var variables = GetVariables();
			...
			MultiProgressDialog.RunAsync(state.TabsWindow, "Splitting files...", Enumerable.Range(0, Selections.Count), (index, progress, cancel) => SplitFileAsync(files[index], outputTemplates[index], chunkSizes[index], progress, cancel), index => Path.GetFileName(files[index]));
		}

		void ConfigureExecute_Files_Operations_CombineFiles() => state.ConfigureExecuteData = FilesOperationsCombineFilesDialog.Run(state.TabsWindow, GetVariables());

		void Execute_Files_Operations_CombineFiles()
		{
			var result = state.ConfigureExecuteData as FilesOperationsCombineFilesDialog.Result;
			var variables = GetVariables();
			...
			MultiProgressDialog.RunAsync(state.TabsWindow, "Combining files...", Enumerable.Range(0, outputFiles.Count), (index, progress, cancel) => CombineFilesAsync(outputFiles[index], inputs[index], progress, cancel), index => Path.GetFileName(outputFiles[index]));
		}
	}
}
