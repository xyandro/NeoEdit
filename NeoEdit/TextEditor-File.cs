...
namespace NeoEdit.Program
{
	partial class TextEditor
	{
		...
		void Command_File_SaveCopy_SaveCopyClipboard(bool copyOnly = false)
		{
			...
			{
				if (!state.SavedAnswers[nameof(Command_File_SaveCopy_SaveCopyClipboard)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_SaveCopy_SaveCopyClipboard)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
				...
				if (!state.SavedAnswers[nameof(Command_File_SaveCopy_SaveCopyClipboard)].HasFlag(MessageOptions.Yes))
					return;
			}
		...
		void Command_File_SaveCopy_SaveCopyByExpression(GetExpressionDialog.Result result, bool copyOnly = false)
		{
			...
			if (File.Exists(newFileName))
			{
				if (!state.SavedAnswers[nameof(Command_File_SaveCopy_SaveCopyByExpression)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_SaveCopy_SaveCopyByExpression)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
						Text = "File already exists; overwrite?",
						Options = MessageOptions.YesNoAllCancel,
				...
				if (!state.SavedAnswers[nameof(Command_File_SaveCopy_SaveCopyByExpression)].HasFlag(MessageOptions.Yes))
					return;
			}
		...
		void Command_File_Operations_RenameClipboard()
		{
			...
			{
				if (!state.SavedAnswers[nameof(Command_File_Operations_RenameClipboard)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_Operations_RenameClipboard)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
						Text = "File already exists; overwrite?",
						Options = MessageOptions.YesNoAllCancel,
				...
				if (!state.SavedAnswers[nameof(Command_File_Operations_RenameClipboard)].HasFlag(MessageOptions.Yes))
					return;
			}
		...
		void Command_File_Operations_RenameByExpression(GetExpressionDialog.Result result)
		{
			...
			if ((!string.Equals(newFileName, FileName, StringComparison.OrdinalIgnoreCase)) && (File.Exists(newFileName)))
			{
				if (!state.SavedAnswers[nameof(Command_File_Operations_RenameByExpression)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_Operations_RenameByExpression)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
						Text = "File already exists; overwrite?",
						Options = MessageOptions.YesNoAllCancel,
				...
				if (!state.SavedAnswers[nameof(Command_File_Operations_RenameByExpression)].HasFlag(MessageOptions.Yes))
					return;
			}
		...
		void Command_File_Operations_Delete()
		{
			...
			if (!state.SavedAnswers[nameof(Command_File_Operations_Delete)].HasFlag(MessageOptions.All))
				state.SavedAnswers[nameof(Command_File_Operations_Delete)] = new Message(state.TabsWindow)
				{
					Title = "Confirm",
			...
			if (!state.SavedAnswers[nameof(Command_File_Operations_Delete)].HasFlag(MessageOptions.Yes))
				return;
		...
		public void Command_File_Refresh()
		{
			...
			{
				if (!state.SavedAnswers[nameof(Command_File_Refresh)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_Refresh)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
				...
				if (!state.SavedAnswers[nameof(Command_File_Refresh)].HasFlag(MessageOptions.Yes))
					return;
		...
		void Command_File_Revert()
		{
			if (IsModified)
			{
				if (!state.SavedAnswers[nameof(Command_File_Revert)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_Revert)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
				...
				if (!state.SavedAnswers[nameof(Command_File_Revert)].HasFlag(MessageOptions.Yes))
					return;
			}
		...
		void Command_File_Insert_CopiedCut()
		{
			...
			if (files.Count > 5)
			{
				if (!state.SavedAnswers[nameof(Command_File_Insert_CopiedCut)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_Insert_CopiedCut)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
				...
				if (!state.SavedAnswers[nameof(Command_File_Insert_CopiedCut)].HasFlag(MessageOptions.Yes))
					return;
			}
		...
		void Command_File_Encoding_ReopenWithEncoding(EncodingDialog.Result result)
		{
			if (IsModified)
			{
				if (!state.SavedAnswers[nameof(Command_File_Encoding_ReopenWithEncoding)].HasFlag(MessageOptions.All))
					state.SavedAnswers[nameof(Command_File_Encoding_ReopenWithEncoding)] = new Message(state.TabsWindow)
					{
						Title = "Confirm",
						Text = "You have unsaved changes. Are you sure you want to reload?",
						Options = MessageOptions.YesNoAll,
				...
				if (!state.SavedAnswers[nameof(Command_File_Encoding_ReopenWithEncoding)].HasFlag(MessageOptions.Yes))
					return;
			}
