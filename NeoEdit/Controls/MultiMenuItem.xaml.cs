...
namespace NeoEdit.Program.Controls
{
	...
	{
		[DepProp]
		public Tabs Tabs { get { return UIHelper<MultiMenuItem>.GetPropValue<Tabs>(this); } set { UIHelper<MultiMenuItem>.SetPropValue(this, value); } }
		[DepProp]
		...
		{
			if ((Property == null) || (Tabs == null))
				MultiChecked = null;
			else
			{
				var property = typeof(TextEditor).GetProperty(Property);
				var match = Tabs.ActiveWindows.Select(obj => property.GetValue(obj)).Select(value => Converter?.Convert(value, MultiValue.GetType(), null, CultureInfo.DefaultThreadCurrentCulture) ?? value).Select(value => value.Equals(MultiValue)).Distinct().ToList();
