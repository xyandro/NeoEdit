...
namespace NeoEdit.Program
{
	public class UndoRedo2
	{
		...
		UndoRedo2(IReadOnlyList<UndoRedoStep> undo, IReadOnlyList<UndoRedoStep> redo)
		{
			this.undo = undo;
			this.redo = redo;
		}

		public static void Clear(ref UndoRedo2 undoRedo)
		{
			undoRedo = new UndoRedo2(new List<UndoRedoStep>(), new List<UndoRedoStep>());
		}

		public static UndoRedoStep GetUndo(ref UndoRedo2 undoRedo)
		{
			if (undoRedo.undo.Count == 0)
				return null;

			var result = undoRedo.undo.Last();
			undoRedo = new UndoRedo2(undoRedo.undo.Take(undoRedo.undo.Count - 1).ToList(), undoRedo.redo);
			return result;
		}

		public static UndoRedoStep GetRedo(ref UndoRedo2 undoRedo)
		{
			if (undoRedo.redo.Count == 0)
				return null;

			var step = undoRedo.redo.Last();
			undoRedo = new UndoRedo2(undoRedo.undo, undoRedo.redo.Take(undoRedo.redo.Count - 1).ToList());
			return step;
		}

		public static void AddUndone(ref UndoRedo2 undoRedo, UndoRedoStep current)
		{
			undoRedo = new UndoRedo2(undoRedo.undo, undoRedo.redo.Concat(current).ToList());
		}

		public static void AddRedone(ref UndoRedo2 undoRedo, UndoRedoStep current)
		{
			undoRedo = new UndoRedo2(undoRedo.undo.Concat(current).ToList(), undoRedo.redo);
		}

		const int maxUndo = 1048576 * 10;
		public static void AddUndo(ref UndoRedo2 undoRedo, UndoRedoStep current, bool modified)
		{
			...
			undoRedo = new UndoRedo2(undo, new List<UndoRedoStep>());
		}
	}
}
