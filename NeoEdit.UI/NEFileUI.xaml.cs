...
namespace NeoEdit.UI
{
	partial class NEFileUI
	{
		...
		void ScrollChanged(object sender, RoutedPropertyChangedEventArgs<double> e) => FilesWindow.HandleCommand(new ExecuteState(NECommand.Internal_Scroll) { Configuration = new Configuration_Internal_Scroll { NEFile = NEFile, Column = (int)xScroll.Value, Row = (int)yScroll.Value } });
		...
		void HandleMouse(Point position, bool? selecting)
		{
			...
			FilesWindow.HandleCommand(new ExecuteState(NECommand.Internal_Mouse) { Configuration = new Configuration_Internal_Mouse { NEFile = NEFile, Line = line, Column = column, ClickCount = mouseClickCount, Selecting = selecting } });
		}
