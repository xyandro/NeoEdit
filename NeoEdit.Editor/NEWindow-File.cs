...
namespace NeoEdit.Editor
{
	partial class NEWindow
	{
		...
		void Execute__File_Select_All()
		{
			ActiveFiles = NEFiles;
			Focused = ActiveFiles.FirstOrDefault();
		}

		void Execute__File_Select_None() => ClearActiveFiles();

		void Execute__File_Select_WithSelections__File_Select_WithoutSelections(bool hasSelections) => SetActiveFiles(ActiveFiles.Where(neFile => neFile.Selections.Any() == hasSelections));

		void Execute__File_Select_Modified__File_Select_Unmodified(bool modified) => SetActiveFiles(ActiveFiles.Where(neFile => neFile.IsModified == modified));

		void Execute__File_Select_ExternalModified__File_Select_ExternalUnmodified(bool modified) => SetActiveFiles(ActiveFiles.Where(neFile => (neFile.LastWriteTime != neFile.LastExternalWriteTime) == modified));

		void Execute__File_Select_Inactive() => SetActiveFiles(NEFiles.Except(ActiveFiles));

		void Execute__File_Select_SelectByExpression()
		{
		...
		void Execute__File_Select_Choose()
		{
		...
		void Execute__File_New_New() => AddNewNEFile(new NEFile());

		void Execute__File_New_FromSelections_All__File_New_FromSelections_Files__File_New_FromSelections_Selections()
		{
		...
		void PostExecute__File_New_FromSelections_All()
		{
		...
		void PostExecute__File_New_FromSelections_Files()
		{
			var preExecution = state.PreExecution as PreExecution_File_New_FromSelections_AllFilesSelections;
		...
		void PostExecute__File_New_FromSelections_Selections()
		{
			var preExecution = state.PreExecution as PreExecution_File_New_FromSelections_AllFilesSelections;
			var index = 0;
		...
		void Execute__File_New_FromClipboard_All()
		{
		...
		void Execute__File_New_WordList()
		{
			byte[] bytes;
		...
		void Configure__File_Open_Open__Macro_Open_Open(string initialDirectory = null)
		{
		...
		void Execute__File_Open_Open__Macro_Open_Open()
		{
		...
		void Execute__File_Open_CopiedCut() => NEClipboard.Current.Strings.AsTaskRunner().Select(file => new NEFile(file)).ForEach(neFile => AddNewNEFile(neFile));

		void Configure__File_Open_ReopenWithEncoding()
		{
		...
		void Configure__File_Select_SelectByExpression__File_Save_SaveAsByExpression__File_Copy_CopyByExpression__File_Advanced_SetDisplayName() => state.Configuration = neWindowUI.RunDialog_Configure_FileTable_Various_Various(Focused?.GetVariables() ?? new NEVariables(), Focused?.Selections.Count ?? 0);

		void Configure__File_Move_MoveByExpression() => state.Configuration = neWindowUI.RunDialog_Configure_FileTable_Various_Various(Focused?.GetVariables() ?? new NEVariables(), Focused?.Selections.Count ?? 0);

		void Configure__File_Encoding() => state.Configuration = neWindowUI.RunDialog_Configure_File_OpenEncoding_ReopenWithEncoding(Focused?.CodePage ?? Coder.CodePage.UTF8, Focused?.HasBOM ?? true);

		void Configure__File_LineEndings()
		{
		...
		void Configure__File_Advanced_Encrypt()
		{
		...
		void Configure__File_Exit() => state.Configuration = new Configuration_File_Exit { ShouldExit = true };

		void Execute__File_Exit()
		{
