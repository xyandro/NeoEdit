...
namespace NeoEdit.Editor
{
	partial class NEFile
	{
		...
		void Execute_Content_HighlightSyntax() => HighlightSyntax = EditorExecuteState.CurrentState.MultiStatus == false;

		void Execute_Content_StrictParsing()
		{
			StrictParsing = EditorExecuteState.CurrentState.MultiStatus == false;
		...
		void Execute_Content_TogglePosition()
		{
			...
			Selections = nodes.Select((node, index) => MoveCursor(Selections[index], allAtBeginning ? node.End : node.Start, EditorExecuteState.CurrentState.ShiftDown)).ToList();
		}
		...
		static void Configure_Content_Ancestor() => EditorExecuteState.CurrentState.Configuration = EditorExecuteState.CurrentState.NEWindowUI.RunDialog_Configure_Content_Various_WithAttribute(EditorExecuteState.CurrentState.NEWindow.Focused.GetSelectionNodes().SelectMany(node => node.Parents()).Distinct().ToList());

		void Execute_Content_Ancestor()
		{
			var result = EditorExecuteState.CurrentState.Configuration as Configuration_Content_Various_WithAttribute;
		...
		static void Configure_Content_Attributes() => EditorExecuteState.CurrentState.Configuration = EditorExecuteState.CurrentState.NEWindowUI.RunDialog_Configure_Content_Attributes(EditorExecuteState.CurrentState.NEWindow.Focused.GetSelectionNodes());

		void Execute_Content_Attributes()
		{
			var result = EditorExecuteState.CurrentState.Configuration as Configuration_Content_Attributes;
			ContentReplaceSelections(GetSelectionNodes().SelectMany(node => node.GetAttrs(result.Attribute, result.FirstOnly)));
		}

		static void Configure_Content_WithAttribute() => EditorExecuteState.CurrentState.Configuration = EditorExecuteState.CurrentState.NEWindowUI.RunDialog_Configure_Content_Various_WithAttribute(EditorExecuteState.CurrentState.NEWindow.Focused.GetSelectionNodes());
		...
		static void Configure_Content_Children_WithAttribute() => EditorExecuteState.CurrentState.Configuration = EditorExecuteState.CurrentState.NEWindowUI.RunDialog_Configure_Content_Various_WithAttribute(EditorExecuteState.CurrentState.NEWindow.Focused.GetSelectionNodes().SelectMany(node => node.Children()).Distinct().ToList());
		...
		static void Configure_Content_Descendants_WithAttribute() => EditorExecuteState.CurrentState.Configuration = EditorExecuteState.CurrentState.NEWindowUI.RunDialog_Configure_Content_Various_WithAttribute(EditorExecuteState.CurrentState.NEWindow.Focused.GetSelectionNodes().SelectMany(node => node.Descendants()).Distinct().ToList());
		...
		void Execute_Content_KeepSelections() => KeepSelections = EditorExecuteState.CurrentState.MultiStatus != true;
	}
}
