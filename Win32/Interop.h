...
namespace NeoEdit
{
	namespace Win32
	{
		...
		public:
			...
			static void ReadProcessMemory(Handle ^handle, int64_t index, array<byte> ^bytes, int bytesIndex, int numBytes);
			static void WriteProcessMemory(Handle ^handle, int64_t index, array<byte> ^bytes, int numBytes);
			...
			static void ReadSharedMemory(int pid, System::IntPtr handle, int64_t index, array<byte> ^bytes, int bytesIndex, int numBytes);
			static void WriteSharedMemory(int pid, System::IntPtr handle, int64_t index, array<byte> ^bytes);
		private:
