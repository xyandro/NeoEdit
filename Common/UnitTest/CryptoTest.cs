...
namespace NeoEdit.Common.UnitTest
{
	public partial class UnitTest
	{
		...
		{
			Assert.AreEqual(Crypto.GetRfc2898Key("Password", "U2FsdHkgR29vZG5lc3M=", 256), "iSE0XUnVlwuKTn0ZWSJo/xE5sZ1vBERwE/PJQ9VPhow=");

			// Make sure keys generate (no exceptions)
			foreach (var type in Helpers.GetValues<Crypto.Type>())
			{
		...
		public void SmallCryptoEncryptTest()
		{
			foreach (var type in Helpers.GetValues<Crypto.Type>())
			{
				...
				if ((type == Crypto.Type.None) || (type == Crypto.Type.DSA))
					continue;
		...
		public void LargeCryptoEncryptTest()
		{
			foreach (var type in Helpers.GetValues<Crypto.Type>())
			{
				...
				if ((type == Crypto.Type.None) || (type == Crypto.Type.RSA) || (type == Crypto.Type.DSA))
					continue;
		...
		public void SmallCryptoSignTest()
		{
			...
			foreach (var type in Helpers.GetValues<Crypto.Type>())
			{
				if ((type == Crypto.Type.None) || (type == Crypto.Type.RSAAES) || (type.IsSymmetric()))
					continue;

				var hashes = type.SigningHashes();
				foreach (var hash in hashes)
				{
					var signature = Crypto.Sign(type, SmallTestData, privKeys[type], hash);
					if (type != Crypto.Type.DSA) // DSA generates a new one every time.
						Assert.AreEqual(signature, correctSignature[type.ToString() + "/" + hash]);
					Assert.IsTrue(Crypto.Verify(type, SmallTestData, pubKeys[type], hash, signature));
				}
			}
		}
		...
		public void LargeCryptoSignTest()
		{
			...
			foreach (var type in Helpers.GetValues<Crypto.Type>())
			{
				...
				foreach (var hash in hashes)
				{
					var signature = Crypto.Sign(type, LargeTestData, privKeys[type], hash);
					if (type != Crypto.Type.DSA) // DSA generates a new one every time.
						Assert.AreEqual(signature, correctSignature[type.ToString() + "/" + hash]);
					Assert.IsTrue(Crypto.Verify(type, LargeTestData, pubKeys[type], hash, signature));
				}
			}
		}
	}
