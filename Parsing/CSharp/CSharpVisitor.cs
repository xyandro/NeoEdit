...
namespace NeoEdit.Parsing
{
	class CSharpVisitor : CSharp4BaseVisitor<object>
	{
		...
		{
			stack.Peek().AddAttr(MODIFIER, input, context);
			return null;
		}

		public override object VisitAttribute_section(CSharp4Parser.Attribute_sectionContext context)
		{
			stack.Peek().AddAttr(ATTRIBUTE, input, context);
			return null;
		}

		public override object VisitBlock(CSharp4Parser.BlockContext context)
		{
			stack.Push(new ParserNode { Type = BLOCK, Parent = stack.Peek(), LocationContext = context });
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.BREAK());
			return base.VisitBreak_statement(context);
		}

		public override object VisitClass_base(CSharp4Parser.Class_baseContext context)
		{
			context.Take(c => c.class_type()).Do(c => stack.Peek().AddAttr(BASE, input, c));
			context.Take(c => c.interface_type()).Do(c => stack.Peek().AddAttr(BASE, input, c));
			return null;
		}

		public override object VisitClass_definition(CSharp4Parser.Class_definitionContext context)
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.CLASS());
		...
		{
			stack.Push(new ParserNode { Parent = stack.Peek(), LocationContext = context });
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.CONST());
		...
		{
			stack.Peek().Type = METHOD;
			stack.Peek().AddAttr(ParserNode.TYPE, CONSTRUCTOR);
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.CONTINUE());
		...
		{
			stack.Push(new ParserNode { Type = DECLARATION, Parent = stack.Peek(), LocationContext = context });
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.DELEGATE());
		...
		{
			stack.Peek().Type = METHOD;
			stack.Peek().AddAttr(ParserNode.TYPE, DESTRUCTOR);
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.DO());
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.EVENT());
		...
		{
			stack.Peek().AddAttr(PARAMETER, input, context);
			return null;
		}

		public override object VisitFixed_statement(CSharp4Parser.Fixed_statementContext context)
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.FIXED());
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.FOREACH());
		...
		{
			context.Take(c => c.interface_type_list()).Take(c => c.interface_type()).Do(c => stack.Peek().AddAttr(BASE, input, c));
			return null;
		}

		public override object VisitInterface_definition(CSharp4Parser.Interface_definitionContext context)
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.INTERFACE());
		...
		{
			stack.Push(new ParserNode { Type = INTERFACEMEMBER, Parent = stack.Peek(), LocationContext = context });
		...
		{
			var node = new ParserNode { Type = LABEL, Parent = stack.Peek(), LocationContext = context };
		...
		{
			stack.Peek().AddAttr(RETURNTYPE, input, context.t);
		...
		{
			stack.Push(new ParserNode { Type = INITIALIZER, Parent = stack.Peek(), LocationContext = context });
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.LOCK());
		...
		{
			var node = new ParserNode { Parent = stack.Peek(), LocationContext = context };
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.OPERATOR());
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.RETURN());
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.STRUCT());
		...
		{
			stack.Peek().AddAttr(CONDITION, input, context);
			return null;
		}

		public override object VisitSwitch_section(CSharp4Parser.Switch_sectionContext context)
		{
			stack.Push(new ParserNode { Type = CASE, Parent = stack.Peek(), LocationContext = context });
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.SWITCH());
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.TRY());
		...
		{
			context.Take(c => c.type()).Do(c => stack.Peek().AddAttr(RETURNTYPE, input, c));
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.UNCHECKED());
		...
		public override object VisitUsing_directives(CSharp4Parser.Using_directivesContext context)
		{
			...
			if (usingDirectives.Length > 1)
				parent = new ParserNode { Type = USINGS, Parent = parent, LocationContext = context };
			foreach (var usingDirective in usingDirectives)
				new ParserNode { Type = USING, Parent = parent, LocationContext = usingDirective };
			return null;
		}

		public override object VisitUsing_statement(CSharp4Parser.Using_statementContext context)
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.USING());
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.WHILE());
		...
		{
			stack.Peek().AddAttr(ParserNode.TYPE, input, context.yield_contextual_keyword());
