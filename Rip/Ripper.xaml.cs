...
namespace NeoEdit.Rip
{
	partial class Ripper
	{
		...
		void Command_Edit_CopyTitles() => NEClipboard.Current = NEClipboard.Create(ripItems.SelectedItems.Cast<RipItem>().Select(item => item.Title).ToList());

		void Command_Edit_CopyFileNames() => NEClipboard.Current = NEClipboard.Create(ripItems.SelectedItems.Cast<RipItem>().Select(item => item.FileName).ToList());
