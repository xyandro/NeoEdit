...
namespace NeoEdit.TaskRunning
{
	...
	public class FluentTaskRunner<T>
	{
		...
		{
			return new FluentTaskRunner<TResult>(nextTask => runTask(new TaskRunnerTask<T, TResult>(getSize, func, (items, results) => nextTask.RunTask(results))));
		}
		#endregion
		...
		{
			return new FluentTaskRunner<TResult>(nextTask => runTask(new TaskRunnerTask<T, IEnumerable<TResult>>(getSize, func, (items, results) =>
			{
		...
		{
			return new FluentTaskRunner<IReadOnlyList<T>>(nextTask => runTask(new TaskRunnerTask<T, T>(null, null, (items, results) =>
			{
		...
		{
			return new FluentTaskRunner<FluentTaskRunnerGroup<TKey, T>>(nextTask => runTask(new TaskRunnerTask<T, TKey>(getSize, func, (items, results) =>
			{
		...
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, bool>(getSize, predicate, (items, results) =>
			{
		...
		{
			bool ret = default;
			runTask(new TaskRunnerTask<T, bool>(getSize, predicate, (items, results) =>
			{
				ret = true;
				foreach (var result in results)
					if (!result)
		...
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, TResult>(getSize, func, (items, results) =>
			{
				var nextList = new List<T>();
		...
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, string>(getSize, func, (items, results) =>
			{
				var nextList = new List<T>();
				for (var ctr = 0; ctr < items.Count; ++ctr)
		...
		public Dictionary<TKey, TValue> ToDictionary<TKey, TValue>(Func<T, int, Action<long>, TKey> keyFunc, Func<T, int, Action<long>, TValue> valueFunc, IEqualityComparer<TKey> comparer = null, Func<T, long> getSize = null)
		{
			...
			runTask(new TaskRunnerTask<T, TKey>(getSize, keyFunc, (items, results) => keys = results));
			runTask(new TaskRunnerTask<T, TValue>(getSize, valueFunc, (items, results) => values = results));
		...
		public IReadOnlyList<T> ToList()
		{
			...
			runTask(new TaskRunnerTask<T, bool>(null, null, (items, results) => result = items));
			return result;
		}
		#endregion
		...
		{
			int result = default;
			runTask(new TaskRunnerTask<T, bool>(null, null, (items, results) => result = items.Count));
			return result;
		}
		#endregion

		#region Distinct
		public FluentTaskRunner<T> Distinct() => new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, T>(null, null, (items, results) => nextTask.RunTask(items.Distinct()))));
		#endregion

		#region DefaultIfEmpty
		public FluentTaskRunner<T> DefaultIfEmpty(T value)
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, T>(null, null, (items, results) =>
			{
				if (items.Count == 0)
		...
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, TResult>(getSize, func, (items, results) => nextTask.RunTask(items.Zip(results, (item, result) => (item, result)).OrderBy(obj => obj.result, comparer ?? Comparer<TResult>.Default).Select(obj => obj.item).ToList()))));
		}
		#endregion
		...
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, TResult>(getSize, func, (items, results) => nextTask.RunTask(items.Zip(results, (item, result) => (item, result)).OrderByDescending(obj => obj.result, comparer ?? Comparer<TResult>.Default).Select(obj => obj.item).ToList()))));
		}
		#endregion
		...
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, T>(null, null, (items, results) => nextTask.RunTask(items.OrderBy(obj => obj, comparer ?? Comparer<T>.Default)))));
		}
		#endregion
		...
		{
			return new FluentTaskRunner<T>(nextTask => runTask(new TaskRunnerTask<T, T>(null, null, (items, results) => nextTask.RunTask(items.OrderByDescending(obj => obj, comparer ?? Comparer<T>.Default)))));
		}
		#endregion
		...
		{
			T result = default;
			runTask(new TaskRunnerTask<T, T>(null, null, (items, results) =>
			{
				if (items.Count == 0)
		...
		public TResult Min<TResult>(Func<T, int, Action<long>, TResult> func, Func<T, long> getSize = null) where TResult : IComparable
		{
			...
			runTask(new TaskRunnerTask<T, TResult>(getSize, func, (items, results) =>
			{
				if (items.Count == 0)
		...
		public void ForAll(Action<T, int, Action<long>> action, Func<T, long> getSize = null) => runTask(new TaskRunnerTask<T, bool>(getSize, (item, index, progress) => { action(item, index, progress); return false; }, null));
		#endregion
	}
}
